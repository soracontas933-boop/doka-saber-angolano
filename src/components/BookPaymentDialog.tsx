...
const BookPaymentDialog = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[98vw] sm:w-[95vw] sm:max-w-2xl max-h-[96vh] overflow-y-auto p-4 sm:p-8 rounded-t-[2rem] sm:rounded-[2.5rem] border-0 shadow-2xl bg-background dark:bg-background bottom-0 sm:bottom-auto translate-y-0 sm:-translate-y-1/2">
        <DialogHeader className="space-y-1 sm:space-y-2 pb-2">
          ...
          <DialogDescription className="text-[11px] sm:text-sm text-muted-foreground">
            Transfira <span className="font-bold text-foreground">{book.preco_kz} Kz</span> e envie o comprovativo.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-secondary p-2.5 sm:p-4 rounded-2xl border border-border flex items-center gap-3 card-3d">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] sm:text-sm text-muted-foreground truncate">{book.autor}</p>
          ...
          {loadingPaymentMethods ? (
            ...
            <div className="space-y-3">
              <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Dados Bancários</p>
              <Tabs value={selectedMethod || ""} onValueChange={setSelectedMethod} className="w-full">
                <TabsList className="flex w-full bg-secondary p-1 rounded-2xl h-11 sm:h-12">
                  {authorPaymentMethods.map((method) => (
                    <TabsTrigger key={method.id} value={method.id} className="flex-1 text-[11px] sm:text-sm font-bold rounded-xl data-[state=active]:bg-card dark:data-[state=active]:bg-card data-[state=active]:shadow-sm transition-all">
                ...
                {authorPaymentMethods.map((method) => {
                  ...
                  return (
                    <TabsContent key={method.id} value={method.id} className="mt-3 sm:mt-4">
                      <div className="bg-card text-foreground p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-3 sm:space-y-4 shadow-xl border border-border button-3d">
                        <div className="flex justify-between items-start">
                          <p className="text-[10px] sm:text-xs text-muted-foreground font-bold uppercase tracking-wider">{display.title}</p>
                          {method.preferido && <Badge className="bg-green-500 hover:bg-green-500 text-[9px] sm:text-[10px] h-4 sm:h-5 px-2">Preferido</Badge>}
                        </div>
                        <div className="bg-secondary p-3 sm:p-4 rounded-xl break-all font-mono text-xs sm:text-base border border-border select-all text-center sm:text-left leading-relaxed text-foreground">
                          {display.value}
                        </div>
                        {display.subtitle && <p className="text-[10px] sm:text-xs text-muted-foreground text-center sm:text-left">{display.subtitle}</p>}
                      </div>
                    </TabsContent>
                  );
                })}
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="space-y-2">
              <Label className="text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-wider">Seu Email para Confirmação</Label>
              <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="exemplo@email.com" className="h-11 sm:h-12 rounded-2xl border-border bg-secondary text-sm sm:text-base focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>

            <div className="space-y-2">
              <Label className="text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-wider">Comprovativo</Label>
              <div
                className={`relative flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed p-5 sm:p-10 transition-all cursor-pointer group ${
                  file ? "border-primary/50 bg-primary/5" : "border-border hover:border-primary/30 hover:bg-secondary"
                }`}
                onClick={() => {
              ...
              >
                {file ? (
                  <div className="flex items-center gap-3 w-full bg-card p-3 rounded-2xl shadow-sm border border-primary/10 button-3d">
                    <div className="bg-primary/10 p-2.5 rounded-xl"><FileText className="h-5 w-5 sm:h-6 sm:w-6 text-primary" /></div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-bold truncate text-foreground">{file.name}</p>
                  ...
                  <>
                    <div className="bg-secondary p-3 sm:p-4 rounded-full mb-3 group-hover:scale-110 transition-transform"><Upload className="h-5 w-5 sm:h-6 sm:w-6 text-muted-foreground group-hover:text-primary" /></div>
                    <p className="text-xs sm:text-base font-bold text-foreground">Carregar ficheiro</p>
                    <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">PNG, JPG ou PDF (máx. 5MB)</p>
                  </>
                )}
              </div>
            </div>
