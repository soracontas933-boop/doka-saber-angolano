// Dot Matrix — Dotted Background Animation
// Adaptado para o projeto KIHAHA
...
export default function DotMatrix({
  ...
  const renderOnce = () => {
    ...
    if (
      !renderer || !camera || !perlinMesh || !dotMesh ||
      !renderTarget || !gl || !dotProgram
    )
      return;
    ...
    if (effectivePlay) {
      if (rafIdRef.current == null) {
        ...
        rafIdRef.current = requestAnimationFrame(function update(
          ...
          const gl = glRef.current;
          if (
            !isPlayingRef.current || !perlin || !dot ||
            !renderer || !camera || !perlinMesh || !dotMesh ||
            !renderTarget || !gl
          ) {
            rafIdRef.current = null;
