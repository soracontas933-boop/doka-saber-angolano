...
import { Loader2, Download, BookOpen, Library } from "lucide-react";
...
const MinhaBibliotecaPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from("book_library").select("*, books(*)").eq("user_id", user.id).order("obtido_em", { ascending: false });
      setItems(data || []);
      setLoading(false);
    };
    load();
