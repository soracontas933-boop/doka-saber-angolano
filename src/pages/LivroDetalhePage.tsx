    ...
    if (book) {
      ...
      const updateMeta = (name: string, content: string, attr = "name") => {
        try {
          ...
          console.error("Erro ao atualizar meta tag:", e);
        }
      };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-6 pb-24 md:pb-6">
        <div className="flex flex-col md:flex-row gap-6 lg:gap-8">
          ...
          {/* Seção da Capa - Mobile: Lado a Lado / Desktop: Esquerda */}
          <div className="flex flex-row md:flex-col gap-4 sm:gap-6 md:w-[280px] lg:w-[320px] flex-shrink-0">
            
            {/* Capa com Efeito */}
            <div className="w-[120px] sm:w-[160px] md:w-full flex-shrink-0">
              <div 
                className="bg-secondary rounded-lg overflow-hidden aspect-[2/3] shadow-md transition-all duration-300 ease-out cursor-pointer active:scale-95 md:hover:scale-105"
            ...
            {/* Info Básica ao lado da capa no Mobile */}
            <div className="flex-1 flex flex-col justify-center md:hidden space-y-2">
              {book.book_categories?.nome && (
                <Badge variant="outline" className="text-[10px] w-fit">{book.book_categories.nome}</Badge>
              )}
              <h1 className="text-base font-bold leading-tight line-clamp-3">
                {book.titulo}
              </h1>
              <p className="text-[11px] text-muted-foreground">por <span className="font-semibold text-foreground">{book.autor}</span></p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="hidden md:block space-y-4">
              ...
              <p className="text-sm text-muted-foreground">por <span className="font-semibold text-foreground">{book.autor}</span></p>
            </div>

            {/* Metadados (Paginas, Idioma, etc) */}
            <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm text-muted-foreground py-3 border-y border-border">
              {book.paginas && (
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold">Sobre o livro</h3>
              ...
              {isDescriptionLong && (
                <button
                  ...
                  className="text-primary text-xs font-medium flex items-center gap-1 mt-1"
                >
                  {expandedDescription ? "Ver menos" : "Ver mais"}
                  <ChevronDown className={`h-3 w-3 transition-transform ${expandedDescription ? 'rotate-180' : ''}`} />
                </button>
              )}
            </div>

            {/* Botões de Ação - Sempre abaixo */}
            <div className="space-y-3 pt-2">
              {owned ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Button onClick={handleRead} disabled={processing} className="rounded-xl gap-2 w-full h-11">
                    {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Eye className="h-4 w-4" />}
                    <span>Ler agora</span>
                  </Button>
                  <Button onClick={handleDownload} variant="outline" disabled={processing} className="rounded-xl gap-2 w-full h-11">
                    {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
                    <span>Baixar PDF</span>
                  </Button>
                </div>
              ) : book.gratuito ? (
                <Button onClick={handleObterGratis} disabled={processing} className="rounded-xl gap-2 w-full h-11">
                  {processing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
                ...
                <div className="flex flex-col gap-3">
                  {book.preco_creditos > 0 && (
                    <Button onClick={handleComprarCreditos} disabled={processing} className="rounded-xl gap-2 w-full h-11">
                  ...
                  <Button 
                    ...
                    variant="outline" 
                    className="rounded-xl gap-2 w-full h-11"
                  >
            ...
            {/* Info Visualizações */}
            <div className="pt-4 border-t border-border flex justify-between text-[10px] text-muted-foreground">
