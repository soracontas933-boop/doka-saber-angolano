        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-[20]">
          {images.map((_, i) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all ${i === current ? "bg-primary scale-125" : "bg-muted-foreground/40"}`}
            />
          ))}
        </div>
      )}
      ...
      <section id="funcionalidades" className="px-4 sm:px-6 md:px-12 py-14 sm:py-20 bg-card border-t border-border">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {features.map((f) => (
              <motion.div key={f.title} variants={item} className="group flex gap-4 p-5 rounded-2xl border border-border bg-background hover:shadow-card-hover transition-shadow duration-200">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
            {(["gratuito", "basico", "intermedio", "profissional", "premium"] as PlanKey[]).map((key) => {
              ...
              return (
                <motion.div key={key} variants={item} className={`relative flex flex-col rounded-2xl border p-5 transition-shadow duration-200 ${isPopular ? "border-primary bg-primary/5 shadow-lg scale-[1.02]" : "border-border bg-card hover:shadow-card-hover"}`}>
