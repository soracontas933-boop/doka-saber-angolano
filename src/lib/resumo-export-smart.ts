/**
 * Resumo Export Smart — Exportação com Paginação Inteligente
 * 
 * Versão melhorada de resumo-export.ts que garante que nenhum conteúdo
 * seja cortado entre páginas na exportação em PDF.
 * 
 * MELHORIAS:
 * - Usa pdf-export-helper-smart para paginação inteligente
...
import { escapeHtml, exportHtmlToPdfSmart } from "@/lib/pdf-export-helper-smart";
...
export async function exportResumoWordSmart(resultado: string, tipoResumo: string, disciplina: string, titleOverride?: string) {
...
// ─── PDF Export — visual capture com paginação inteligente ─────────────

export async function exportResumoVisualPDFSmart(
  ...
  header.style.cssText =
    "text-align:center;margin-bottom:16px;border-bottom:2px solid #1E9DF1;padding-bottom:10px;overflow:visible;page-break-inside:avoid;";
  header.innerHTML = `
  ...
  footer.style.cssText =
    "margin-top:16px;padding-top:8px;border-top:1px solid #e2e8f0;text-align:center;font-size:9px;color:#94a3b8;overflow:visible;page-break-inside:avoid;";
...
export async function exportResumoPDFSmart(resultado: string, tipoResumo: string, disciplina: string, titleOverride?: string, numPaginas: number = 1) {
  const { title: parsedTitle, sections } = parseResumo(resultado);
  const title = titleOverride || parsedTitle || tipoResumo;
  ...
  // Header
  html += `
    <div style="text-align:center;margin-bottom:24px;position:relative;z-index:1;overflow:visible;page-break-inside:avoid;">
  ...
  // Student fields
  html += `
    <div style="display:flex;justify-content:space-between;margin-bottom:22px;font-size:10pt;overflow:visible;page-break-inside:avoid;">
  ...
  // Content
  if (sections.length === 0) {
    html += `<div style="font-size:11pt;white-space:pre-wrap;overflow:visible;page-break-inside:avoid;">${escapeHtml(resultado)}</div>`;
  } else {
    for (const section of sections) {
      html += `<div style="margin-bottom:16px;break-inside:avoid;page-break-inside:avoid;overflow:visible;">`;
      if (section.heading) {
        html += `<div style="margin-bottom:6px;padding-bottom:3px;${section.level === 2 ? "border-bottom:1px solid #ccc;" : ""}overflow:visible;page-break-inside:avoid;">`;
      ...
      for (const item of section.items) {
        ...
        if (section.heading) {
          html += `<div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:4px;padding-left:12px;font-size:11pt;break-inside:avoid;page-break-inside:avoid;overflow:visible;">`;
          html += `<span style="color:#10b981;font-weight:bold;flex-shrink:0;">•</span>`;
          html += `<span style="text-align:justify;">${safeItem}</span>`;
          html += `</div>`;
        } else {
          html += `<p style="margin-bottom:4px;font-size:11pt;text-align:justify;break-inside:avoid;page-break-inside:avoid;overflow:visible;">${safeItem}</p>`;
        }
      }
      html += `</div>`;
    }
  ...
  // Footer
  html += `<div style="border-top:1px solid #ccc;margin-top:28px;padding-top:10px;text-align:center;font-size:9pt;color:#888;overflow:visible;page-break-inside:avoid;">Gerado por Delle — Plataforma de Estudo Inteligente</div>`;

  // Criamos um elemento temporário para renderizar o HTML
